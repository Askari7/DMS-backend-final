...
// module.exports.resolvedComment = async (req, res) => {
//   try {
//       console.log(req.body, "body");
//       const comment_id = req.body.comments.id;

//       // Fetch the comment matching the given id
//       const matchedComment = await CommentsModel.findOne({
//           where: {comments:{
//               id: comment_id
//             }
//           }
//       });
//         console.log(matchedComment,"matched")
//       if (matchedComment) {
//           // Update the resolved field within the JSON object
...
//       where: { docName: req?.query?.docName, Resolved : !1 },
//     });
//     console.log(comments,"comments");
//     let commentsObject = []
//     if (req.query.user == "1") {
...
module.exports.FetchDepartmentProjectsMDRUsersAndDocuments = async (req, res) => {
 try{
...
module.exports.FetchMDRAndDocuments = async (req, res) => {
  try {

    const companyData = await MDRModel.findAll({
      where: { companyId: req.query.companyId },
      include: [
        {
          model: DocumentModel,
...
module.exports.createDocument = async (req, res) => {
  try {
    ...
    // console.log(req.query.documentInfo);    
    // console.log(req.query.version);
    
    // console.log("data ha upar ka ");
  ...
  console.log(count);
    ...
    console.log('created',document);
      ...
      req.body.docDepartmentId=req.body.departmentId;
      req.body.masterDocumentCode=req.body.masterDocumentId;
      console.log(appName,revName,"namess");
      req.body.approverId = appName.join(', ');
      req.body.reviewerId = revName.join(', ');
      console.log(req.body.approverId,req.body.reviewerId);
      req.body.approverComment = Array.from({ length: appName.length }).map(() => '').join(', ');
      req.body.reviewerComment  = Array.from({ length: revName.length }).map(() => '').join(', ');
      req.body.approverStatus = Array.from({ length: appName.length }).map(() => 0).join(', ');
      req.body.reviewerStatus = Array.from({ length: revName.length }).map(() => 0).join(', ');
...
module.exports.fetchAllEstablishments = async (req, res) => {
  try {
